    ...
    (if (null? S)
        ...
        (begin
          (cond
            ...
            [(and (equal? (car pair) (caar S)) (not (equal? (cdr pair) (cadar S)))) #t]
            [else (has-neighbour? (cdr S))])))))
    ...
    (if (null? S)
        ...
        (begin
          ...
          (cond
            ...
            [(and (and (not (Symmetric? (list (car S)))) (has-neighbour? (car S) S)) (member? (list (cadar S) (cadr (get-neighbour (car S) S))) S))
             (Transitive? (remove-pairs (append (cons (list (car S) (get-neighbour (car S) S)) S) (list (list (cadar S) (cadr (get-neighbour (car S) S)))))))]
